...
async function injectVolumeSlider() {
  ...
  const audio = document.getElementById("audible-player");

  audioProgress.style.width = `${progress}%`;
  audio.volume = perceptualToAmplitude(progress);

  audioSlider.oninput = (event) => {
    const currProgress = event.target.value;
    audioProgress.style.width = `${currProgress}%`;
    audio.volume = perceptualToAmplitude(currProgress);
    window.localStorage.setItem("progress", currProgress);
  };

  ["mousedown", "mousemove", "mouseup", "mouseenter", "mouseleave", "focus", "blur"].forEach(evt => {
